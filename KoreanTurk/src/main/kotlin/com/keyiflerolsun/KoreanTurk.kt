...
class KoreanTurk : MainAPI() {
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val description     = document.selectFirst("[property='og:title']")?.attr("content")?.substringAfter(" Türler: ")?.trim()
        val tags            = document.selectFirst("[property='og:title']")?.attr("content")?.substringBefore(" Türler: ")?.trim()?.split(",")?.mapNotNull { it.trim() }
