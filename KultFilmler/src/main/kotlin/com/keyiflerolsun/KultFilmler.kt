...
class KultFilmler : MainAPI() {
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val year        = Regex("""\d+""").find(document.selectFirst("li.release")?.text()?.trim())?.toIntOrNull()
        val duration    = Regex("""\d+""").find(document.selectFirst("li.time")?.text()?.trim())?.toIntOrNull()
