...
class DiziMom : MainAPI() {
    ...
    override suspend fun loadLinks(data: String, isCasting: Boolean, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit): Boolean {
        ...
        val main_iframe = document.selectFirst("div.video iframe")?.attr("src") ?: return false
        iframes.add(main_iframe)

        document.select("div.sources a").forEach {
            val sub_document = app.get(it.attr("href"), headers=ua).document
            val sub_iframe   = sub_document.selectFirst("div.video iframe")?.attr("src") ?: return@forEach
